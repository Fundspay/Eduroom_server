...
module.exports = (sequelize, Sequelize) => {
  const User = sequelize.define(
    "User",
    {
      ...
      // 🔹 Course Tracking
      courseDates: { type: Sequelize.JSON, allowNull: true, defaultValue: {} },
      courseStatuses: { type: Sequelize.JSON, allowNull: true, defaultValue: {} },
      triggeredTargets: { type: Sequelize.JSON, allowNull: true, defaultValue: {} },
      ...
      // ─────────────────────────────────────────────
      // 🔹 Marketing Metrics — submitted by intern, verified by team manager
      // ─────────────────────────────────────────────

      // Number of qualified leads generated by the intern
      qualifiedLeads: {
      ...
      // Number of reviews collected (e.g. Google, social)
      reviews: {
        type: Sequelize.INTEGER,
        allowNull: true,
        defaultValue: null,
      ...
      // Average rating given (e.g. 4.5 out of 5)
      ratings: {
      ...
      // Followers gained (raw count — coins calculated as per 10 followers)
      followersGrowth: {
        type: Sequelize.INTEGER,
        allowNull: true,
        defaultValue: null,
      ...
      // Verification status of marketing metrics
      // pending  — intern submitted, waiting for manager to verify
      // approved — team manager approved the values
      // rejected — team manager rejected, intern must resubmit
  ...
  User.associate = (models) => {
    ...
    User.hasMany(models.OfferLetter, { foreignKey: "userId", onDelete: "CASCADE", onUpdate: "CASCADE" });
    User.hasMany(models.InternshipCertificate, { foreignKey: "userId", onDelete: "CASCADE", onUpdate: "CASCADE" });
    User.hasMany(models.FundsAudit, { foreignKey: "userId", onDelete: "CASCADE", onUpdate: "CASCADE" });
  };
