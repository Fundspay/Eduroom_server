...
// =======================
// FETCH ALL CASE STUDY RESULTS FOR USER
// =======================
const fetchCaseStudyResults = async ({ courseId, userId }) => {
...
const generateMCQCaseStudyReport = async (options = {}) => {
  ...
  const caseStudyResults = userId
    ? await fetchCaseStudyResults({ courseId, userId })
    : [];
  ...
  // --- Case Study Results Displayed at End ---
  const caseStudyResultsHtml =
    caseStudyResults.length > 0
      ? `
      <div class="page">
        <div class="content">
          <h1>Case Study Results</h1>
          ${caseStudyResults
            .map(
              (cs, idx) => `
            <p><b>Question ID ${cs.questionId}:</b> ${escapeHtml(cs.answer)}</p>
            <p>Match %: ${cs.matchPercentage} | Passed: ${cs.passed ? "Yes" : "No"}</p>
          `
            )
            .join("")}
        </div>
