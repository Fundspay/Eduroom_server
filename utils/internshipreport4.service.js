...
// =======================
// SPLIT LONG TEXT INTO CHUNKS OF MAX WORDS PER PAGE
// =======================
const splitTextByWords = (text, maxWords = 400) => {
...
const generateMCQCaseStudyReport = async (options = {}) => {
  ...
  // =======================
  // CASE STUDY HTML WITH WORD SPLIT
  // =======================
  const caseStudiesHtml =
    allCaseStudies.length > 0
      ? allCaseStudies
          .map((session) => {
            const pages = [];
            session.caseStudies.forEach((cs, idx) => {
              const answerChunks = splitTextByWords(cs.userAnswer || "N/A", 400);
