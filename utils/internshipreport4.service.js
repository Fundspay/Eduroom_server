...
// =======================
// HELPER TO SPLIT TEXT INTO CHUNKS OF MAX 450 WORDS
// =======================
const splitTextByWords = (text, maxWords = 450) => {
...
const fetchSessionsWithMCQs = async (courseId) => {
  ...
  const sessions = courseDetailRows.map((session) => {
    ...
    const suffix = dayCounter[day] === 0 ? "" : `.${dayCounter[day]}`;
    dayCounter[day] += 1;
    ...
    return {
      ...
      title: session.title || `Session ${day}${suffix}`,
    ...
    sessionsWithDates.length > 0
          ...
          .map((s, index) => {
            ...
            const scoreTableHtml = `
              <div style="text-align:center; width:100%;">
                <table border="1" cellpadding="6" cellspacing="0" style="margin-top:15px; border-collapse: collapse; width:70%; margin-left:auto; margin-right:auto;">
            ...
            const courseInfoHtml = index === 0 ? `
              ...
              <p><b>Internship:</b> ${escapeHtml(courseName)}</p>
      ...
      : `<div class="page">
          <div class="content">
            ...
            <p><b>Domain:</b> ${escapeHtml(domain)}</p>
            <p><b>Internship:</b> ${escapeHtml(courseName)}</p>
            <p>There is no data available for this internship.</p>
          </div>
          <div class="footer">Generated by FundsWeb · ${generatedOn}</div>
        </div>`;

  // =======================
  // CASE STUDY HTML WITH SPLIT ANSWERS
  // =======================
 const caseStudiesHtml =
  allCaseStudies.length > 0
    ? allCaseStudies
        .map((session) => {
          return session.caseStudies
            .map((cs, idx) => {
              const answerChunks = splitTextByWords(cs.userAnswer || "N/A", 380);
              return answerChunks
                .map((chunk, chunkIdx) => {
                  const showResultTable = chunkIdx === answerChunks.length - 1;
                  // Only show "Answer Text:" on the first chunk
                  const answerLabel = chunkIdx === 0 ? "<p><b>Answer Text:</b></p>" : "";
                  return `
                    <div class="page">
                      <div class="content">
                        ${chunkIdx === 0 ? `<p><b>Question ${idx + 1}:</b> ${escapeHtml(cs.question)}</p>` : ""}
                        ${answerLabel}
                        <p>${escapeHtml(chunk)}</p>
                        ${
                          showResultTable
                            ? `<div style="text-align:center; width:100%;">
                                <table border="1" cellpadding="6" cellspacing="0" style="margin-top:10px; border-collapse: collapse; width:70%; margin-left:auto; margin-right:auto;">
                                  <tr style="background:#f0f0f0; text-align:center;">
                                    <th>Match %</th>
                                    <th>Passed</th>
                                  </tr>
