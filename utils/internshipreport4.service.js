...
// Fetch all case study results for a course & user using latest submission per question
const fetchAllCaseStudies = async ({ courseId, coursePreviewId, userId }) => {
  if (!userId) return [];

  // fetch latest result per question
  ...
  results.forEach((res) => {
    const key = `${res.day}-${res.sessionNumber}-${res.questionId}`;
    if (!latestResultsMap[key]) {
      latestResultsMap[key] = res;
    }
  });

  return Object.values(latestResultsMap).map((res) => ({
    day: res.day,
...
const generateMCQCaseStudyReport = async (options = {}) => {
  ...
  // Generate HTML for all case studies at the end (MCQ-style table)
